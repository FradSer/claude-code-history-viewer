  ...
  return (
    <div className="mt-2 p-3 bg-orange-50 border border-orange-200 rounded-lg">
      <div className="flex items-center space-x-2 mb-3">
        ...
        <span className="font-medium text-orange-800">{t("gitWorkflow.title")}</span>
        {command && (
      ...
      {status && (
        <div className="mb-2 text-sm text-orange-700">
          <span className="font-medium">{t("gitWorkflow.status")}</span> {String(status)}
        </div>
      )}
      ...
      {Array.isArray(files) && files.length > 0 && (
        <details className="mb-2">
          <summary className="cursor-pointer text-orange-700 font-medium text-sm">
            {t("gitWorkflow.changedFiles", { count: files.length })}
          </summary>
