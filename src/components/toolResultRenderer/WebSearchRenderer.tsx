  ...
  return (
    <div className="mt-2 p-3 bg-blue-50 border border-blue-200 rounded-lg">
      <div className="flex items-center space-x-2 mb-2">
        ...
        <span className="font-medium text-blue-800">{t("webSearch.title")}</span>
      </div>

      {/* 검색 정보 */}
      <div className="mb-3">
        <div className="text-xs font-medium text-gray-600 mb-1">{t("webSearch.query")}</div>
      ...
      {durationSeconds && (
        <div className="mb-3 text-xs">
          <div className="bg-white p-2 rounded border">
            <div className="text-gray-600">{t("webSearch.duration")}</div>
            <div className="font-medium text-blue-600">
              {t("tokens.unit", { value: durationSeconds.toFixed(2) })}
            </div>
          </div>
        </div>
      )}
      ...
      {results.length > 0 && (
        <div>
          <div className="text-xs font-medium text-gray-600 mb-2">
            {t("webSearch.results", { count: results.length })}
          </div>
              ...
              >
                {typeof result === "string" ? (
                  ...
                  <div className="text-sm text-gray-500 italic">
                    {t("hardcoded.unknownResultFormat")}
                  </div>
                )}
              </div>
            ))}
