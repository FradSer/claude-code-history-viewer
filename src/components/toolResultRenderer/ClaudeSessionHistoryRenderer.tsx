      ...
      return (
        <div className="mt-2 p-3 bg-gray-50 border border-gray-200 rounded-lg">
          <div className="flex items-center space-x-2 mb-2">
            ...
            <span className="font-medium text-gray-800">{t("chatHistory.title")}</span>
          </div>
          <p className="text-gray-600 text-sm">
            {t("chatHistory.noValidMessages")}
          </p>
        </div>
      );
    }
      ...
      <div className="mt-2 p-3 bg-purple-50 border border-purple-200 rounded-lg">
        <div className="flex items-center space-x-2 mb-3">
          ...
          <span className="font-medium text-purple-800">
            {t("chatHistory.titleWithCount", { count: chatMessages.length })}
          </span>
        </div>
            ...
            >
              <div className="flex items-center space-x-2 mb-2">
                ...
                <span className="font-medium text-sm">
                  {msg.type === "user" ? t("chatHistory.userType") : t("chatHistory.assistantType")}
                </span>
              ...
              <div className="text-sm">
                ...
                "content" in msg.message ? (
                  ...
                  <span className="text-gray-500 italic">{t("common.noContent")}</span>
                )}
              </div>
              {typeof msg.message === "object" &&
                msg.message !== null &&
                  ...
                  <div className="mt-2 text-xs text-gray-600">
                    ...
                    {"model" in msg.message &&
                      typeof msg.message.model === "string" && (
                        <span className="ml-2">{t("common.model")}: {msg.message.model}</span>
                      )}
                  </div>
                )}
            </div>
    ...
    return (
      <div className="mt-2 p-3 bg-red-50 border border-red-200 rounded-lg">
        <div className="flex items-center space-x-2 mb-2">
          ...
          <span className="font-medium text-red-800">{t("chatHistory.parseError")}</span>
        </div>
        <p className="text-red-600 text-sm">
          {t("chatHistory.parseErrorMessage")}
        </p>
        <details className="mt-2">
          <summary className="text-sm cursor-pointer">{t("common.viewOriginalData")}</summary>
